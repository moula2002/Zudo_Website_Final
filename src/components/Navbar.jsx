...
export default function Navbar({ cartCount = 0, wishlistCount = 0, onLoginClick, onNavigate, onSearch, onCategoryClick, onNavigateToProduct, currentPage = 'home', isB2B = false, onLogout, user, categories = [], subcategories = [], allProducts = [], isDarkMode, toggleTheme }) {
  ...
  const [isLocationModalOpen, setIsLocationModalOpen] = useState(!localStorage.getItem('selectedCity'));
  ...
  const displayLocation = selectedCity || (primaryAddress ? (primaryAddress.city || primaryAddress.address?.split(',')[0]) : (liveCity || 'Select Location'));
            ...
            <div className="hidden sm:flex flex-col ml-2 border-l border-gray-100 pl-4">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.removeItem('zudo_tenant_id');
...
function LocationModal({ isOpen, onClose, selectedCity, setSelectedCity, refreshLocation, locationLoading }) {
  const [locations, setLocations] = useState([]);
  const [pincode, setPincode] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [step, setStep] = useState('pincode'); // 'pincode', 'city', or 'request'
  ...
  const handlePincodeSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // If we were waiting for a specific city, check if it matches
        if (pendingCity && data.city.toLowerCase() !== pendingCity.city.toLowerCase()) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-[#f8f9fa] dark:bg-[#0a0a0a]">
      {/* Background Decor */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-emerald-500/10 rounded-full blur-[120px] animate-pulse"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-emerald-600/10 rounded-full blur-[120px] animate-pulse delay-700"></div>

      <div className="bg-white dark:bg-[#121212] w-full max-w-xl md:rounded-[40px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] relative z-10 overflow-hidden border border-gray-100 dark:border-white/5 h-full md:h-auto max-h-screen flex flex-col">
        <div className="p-8 md:p-10 border-b border-gray-100 dark:border-white/5 flex items-center justify-center text-center">
          <div>
            <div className="w-16 h-16 bg-emerald-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-xl shadow-emerald-600/20 rotate-3 hover:rotate-0 transition-transform">
              <MapPin size={32} className="text-white" />
            </div>
            <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">
              {step === 'city' ? 'Where are you?' : step === 'request' ? 'Request Service' : 'Welcome to Zudo'}
            </h2>
            <p className="text-gray-400 text-xs font-bold uppercase tracking-[0.2em] mt-3">
              {step === 'request' ? 'Service not yet available' : 'Please verify your delivery area to continue'}
            </p>
          </div>
        </div>
        ...
        <div className="p-8 md:p-10 space-y-8 flex-1 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
              {!requestSent ? (
                <>
                  <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl">
                    <p className="text-xs font-bold text-amber-800 leading-relaxed">
                      Sorry! We don't deliver to <span className="font-black underline">{pincode}</span> yet. 
                      Tell us where you are, and we'll notify you when we expand!
                    </p>
                  </div>
                  <form onSubmit={handleRequestSubmit} className="space-y-4">
                    <input 
                      ...
                      className="w-full bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl py-4 px-5 outline-none font-bold text-sm focus:ring-2 focus:ring-emerald-500 transition-all dark:text-white"
                    />
                    <input 
                      type="email" 
                    ...
                    <button 
                      type="submit"
                      disabled={loading}
                      className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-600/20 active:scale-95 disabled:opacity-50"
                    >
                      {loading ? 'Submitting...' : 'Request Store in my Area'}
                    </button>
                  </form>
                  <button onClick={() => setStep('pincode')} className="w-full text-[10px] font-black uppercase text-gray-400 hover:text-emerald-600 transition-all">
                    Try another pincode
                  </button>
                </>
              ) : (
                <div className="text-center py-8 space-y-4">
                  ...
                  <h3 className="text-lg font-black text-gray-900 dark:text-white">Request Received!</h3>
                  <p className="text-xs text-gray-500 leading-relaxed font-bold">
                    Thank you for your interest! We've saved your request for pincode <span className="text-emerald-600 font-black">{pincode}</span>. 
                    We'll email you at <span className="text-emerald-600 font-black">{requestData.email}</span> as soon as we're ready.
                  </p>
                  <button onClick={() => {
                    ...
                    setPincode('');
                    setError('');
                  }} className="mt-6 px-8 py-3 bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-600 transition-all">
            ...
            <>
              ...
              <form onSubmit={handlePincodeSubmit} className="relative">
                <input 
                  ...
                  maxLength="6"
                  autoFocus
                  placeholder={pendingCity ? `Enter Pincode for ${pendingCity.city}` : "Enter Pincode (e.g. 560001)"}
                  value={pincode}
                ...
                <button 
                  type="submit"
                  disabled={loading}
                  className="absolute right-2 top-1/2 -translate-y-1/2 bg-emerald-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all disabled:opacity-50"
                >
                  {loading ? '...' : 'Verify'}
                </button>
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center"><span className="bg-white dark:bg-[#121212] px-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Or</span></div>
              </div>

              <button 
                onClick={() => setStep('city')}
                className="w-full py-4 rounded-2xl border border-gray-100 dark:border-white/10 text-xs font-black uppercase tracking-widest text-gray-500 hover:text-emerald-600 hover:border-emerald-100 transition-all"
              >
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {locations.length > 0 ? locations.map(loc => (
                  <button 
                    key={loc._id || loc.dbName}
                    onClick={() => {
                      setPendingCity(loc);
                      setStep('pincode');
                      setError('');
                    }}
                    className={`p-4 rounded-2xl border text-left transition-all group ${selectedCity === loc.city ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/10 hover:border-emerald-200 text-gray-800 dark:text-gray-200'}`}
                  >
                    <p className={`text-xs font-black uppercase tracking-widest ${selectedCity === loc.city ? 'text-white' : 'text-gray-500 dark:text-gray-400 group-hover:text-emerald-600'}`}>{loc.city}</p>
                    <p className="text-[9px] font-bold mt-1 opacity-60">Manual Select</p>
                  </button>
                )) : (
                  <div className="col-span-2 py-8 text-center text-gray-400 font-bold text-xs">
              ...
              <button 
                ...
                className="w-full py-3 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-emerald-600 transition-all"
              >
                ← Back to Pincode
              </button>
            </div>
          ...
          {/* Live Location Option (Only in Pincode step) */}
          {step === 'pincode' && (
            <button 
              onClick={async () => {
                setError('');
                const loc = await refreshLocation();
                console.log('[DEBUG] Live Location Detected:', loc);
                if (loc && loc.pincode) {
                  console.log('[DEBUG] Validating detected pincode:', loc.pincode);
                  ...
                  try {
                    ...
                    if (res.ok) {
                      ...
                      setPincode(loc.pincode.replace(/\s/g, ''));
                      setStep('request');
                    } else {
                      setError(`Detected ${loc.city} (${loc.pincode}), but: ${data.message || 'Location not found'}`);
                    }
                  } catch (err) {
                    setError('Error validating live location pincode');
                  } finally {
                    setLoading(false);
                  }
                } else if (loc && loc.city) {
                  // Fallback if no pincode but has city (less secure)
                  setSelectedCity(loc.city);
                }
              }}
              className="w-full flex items-center gap-4 p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 group hover:bg-emerald-600 transition-all"
            >
              <div className="w-12 h-12 bg-white dark:bg-emerald-600 rounded-xl flex items-center justify-center text-emerald-600 dark:text-white shadow-sm group-hover:scale-110 transition-transform">
                <MapPin size={24} />
              </div>
              <div className="text-left">
                <p className="text-sm font-black text-emerald-900 dark:text-emerald-400 group-hover:text-white">Use Live Location</p>
                <p className="text-[10px] font-bold text-emerald-700/60 dark:text-emerald-400/60 group-hover:text-white/60 uppercase tracking-widest">Detect current city</p>
              </div>
              {(locationLoading || loading) && <div className="ml-auto w-4 h-4 border-2 border-emerald-600 border-t-transparent rounded-full animate-spin"></div>}
            </button>
          )}
        </div>
        ...
        <div className="p-6 bg-gray-50 dark:bg-white/5 text-center">
          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">
            Pincode verification required for delivery. <br /> More locations coming soon!
          </p>
        </div>
      </div>
    </div>
